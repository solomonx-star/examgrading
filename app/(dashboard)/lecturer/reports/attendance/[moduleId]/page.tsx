...
export default async function LecturerAttendanceReportViewPage({
  params,
  searchParams,
}: {
  ...
  const csvHref = `/api/lecturer/reports/attendance/${moduleId}/csv${
    sp.from || sp.to
      ? `?${[sp.from ? `from=${sp.from}` : null, sp.to ? `to=${sp.to}` : null]
          .filter(Boolean)
          .join("&")}`
      : ""
  }`;
    ...
    <div className="print:bg-white">
      ...
      <section className="mb-6 rounded-2xl border border-stroke bg-white p-5 shadow-sm print:hidden">
        ...
        <p className="mt-1 mb-3 text-sm text-body">
          ...
          {sp.from || sp.to ? ` from ${sp.from ?? "…"} to ${sp.to ?? "…"}` : ""}
          .
        </p>
