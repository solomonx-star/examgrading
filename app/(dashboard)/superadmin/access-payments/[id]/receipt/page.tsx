...
export default async function SuperAdminAccessReceiptPage({
  params,
}: {
  ...
  await connectDB();
  const payment = await AccessPayment.findById(id).lean();
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-4 flex items-center gap-3 print:hidden">
        <Link
          href="/superadmin/access-payments"
      ...
      <section className="rounded-2xl border border-stroke bg-white p-6 shadow-sm print:rounded-none print:border-0 print:shadow-none">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            <p className="text-xs uppercase text-body">Department</p>
            <p className="font-semibold text-foreground">
              {student?.department ?? "—"}
            </p>
          </div>
